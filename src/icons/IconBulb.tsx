import React from "react";

export default function IconBulb(props: React.ComponentPropsWithoutRef<"svg">) {
  return (
    <svg
      {...props}
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g id="Header/Environment/Property 24">
        <g id="Vector">
          <path
            d="M6 9C6 5.68629 8.68629 3 12 3C15.3137 3 18.0002 5.68629 18.0002 9C18.0002 10.2145 17.6394 11.3447 17.019 12.2893C16.0651 13.7418 15.5878 14.4676 15.5259 14.5761C14.9751 15.541 15.0771 15.2007 15.0078 16.3096C15 16.4342 15 16.6229 15 17C15 17.5523 14.5523 18 14 18L10 18C9.44772 18 9 17.5523 9 17C9 16.6229 8.99998 16.4342 8.99219 16.3096C8.92287 15.2007 9.02428 15.541 8.47352 14.5761C8.41161 14.4676 7.93507 13.7418 6.98117 12.2893C6.36084 11.3447 6 10.2145 6 9Z"
            fill="currentColor"
          />
          <path
            d="M9 21H15M12 3C8.68629 3 6 5.68629 6 9C6 10.2145 6.36084 11.3447 6.98117 12.2893C7.93507 13.7418 8.41161 14.4676 8.47352 14.5761C9.02428 15.541 8.92287 15.2007 8.99219 16.3096C8.99998 16.4342 9 16.6229 9 17C9 17.5523 9.44772 18 10 18L14 18C14.5523 18 15 17.5523 15 17C15 16.6229 15 16.4342 15.0078 16.3096C15.0771 15.2007 14.9751 15.541 15.5259 14.5761C15.5878 14.4676 16.0651 13.7418 17.019 12.2893C17.6394 11.3447 18.0002 10.2145 18.0002 9C18.0002 5.68629 15.3137 3 12 3Z"
            stroke="currentColor"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
          />
        </g>
      </g>
    </svg>
  );
}
